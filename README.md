...
### 📦 Prebuilt RPM (easiest)

Go to [Releases](https://github.com/philyuchkoff/HAProxy-3-RPM-builder/releases/latest), download the latest `.rpm` and install:

    sudo dnf -y install ./haproxy-*.el9.x86_64.rpm

New versions are detected and built automatically every day.
...
### ✅ Install:
    ...
    sudo dnf -y install /opt/HAProxy-3-RPM-builder/rpmbuild/RPMS/x86_64/haproxy-*.el9.x86_64.rpm

or, if you build *.rpm with Docker:

    sudo dnf -y install RPMS/haproxy-*.el9.x86_64.rpm 
...
### 🤖 Automatic builds
The [release workflow](.github/workflows/release.yml) checks for new HAProxy versions daily and publishes prebuilt RPMs on [GitHub Releases](https://github.com/philyuchkoff/HAProxy-3-RPM-builder/releases). RPMs are built on AlmaLinux 9 with Prometheus metrics enabled.
